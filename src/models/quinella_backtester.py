"""
Keiba AI Prediction System - Quinella Backtester

Walkforward backtesting for quinella (馬連) betting with actual odds.
"""
...
from .types import QuinellaBetResult, QuinellaBacktestResults
...
class QuinellaBacktester:
    """
    Walkforward backtesting with actual quinella odds.
    ...
    def run_backtest(
        ...
        Args:
            ...
            quinella_lookup: Dict mapping race_id to (horse1, horse2, odds)

        Returns:
            QuinellaBacktestResults with all bet outcomes
        """
        results = QuinellaBacktestResults()

        race_groups = test_df.groupby(RACE_ID_COL)
        logger.info(f"Backtesting quinella on {len(race_groups)} races...")

        for race_id, race_df in race_groups:
            if race_id not in quinella_lookup:
                continue

            actual_h1, actual_h2, actual_odds = quinella_lookup[race_id]
            ...
            bets = self._evaluate_race(
                race_df, model, actual_h1, actual_h2, actual_odds, race_meta
            )
    ...
    def _evaluate_race(
        ...
        actual_odds: float,
        race_meta: Dict,
    ) -> List[QuinellaBetResult]:
        """Evaluate quinella betting opportunities for a single race."""
        bets = []
        ...
        quinella_probs = self.quinella_calc.calculate_quinella_probs(position_probs)

        # Get top quinellas by probability
        top_quinellas = self.quinella_calc.get_top_quinellas(
            quinella_probs, n=self.max_bets_per_race
        )

        # Actual winning set
        actual_set = frozenset({actual_h1, actual_h2})

        for horse_set, pred_prob in top_quinellas:
    ...
    def run_walkforward_backtest(
    ...
    ) -> Tuple[QuinellaBacktestResults, List[Dict]]:
        """Run walkforward backtesting for quinella."""
        logger.info("=" * 60)
        logger.info("QUINELLA WALKFORWARD BACKTEST")
        ...
        quinella_df = self.odds_loader.get_quinella_results()
        quinella_lookup = {
            row["レースID"]: (row["quinella_horse1"], row["quinella_horse2"], row["quinella_odds"])
        ...
        all_results = QuinellaBacktestResults()
        period_results = []
        ...
        for period in range(n_periods):
            ...
            period_result = self.run_backtest(test_df, model, quinella_lookup)
...
def main():
    ...
    print("\n" + "=" * 60)
    print("QUINELLA BACKTEST RESULTS")
    print("=" * 60)
    ...
    # Random quinella probability for 14 horses = 2 / (14 * 13) ≈ 1.1%
    random_prob = 2 / (14 * 13)
    our_hit_rate = results.hit_rate / 100
    print(f"\nRandom quinella probability: {random_prob:.2%}")
