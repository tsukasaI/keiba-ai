"""
Keiba AI Prediction System - Wide Backtester

Walkforward backtesting for wide (ワイド) betting with actual odds.
"""
...
from .types import WideBetResult, WideBacktestResults
...
class WideBacktester:
    """
    Walkforward backtesting with actual wide odds.

    Wide = 2 horses both in top 3, order doesn't matter.
    Multiple winners per race (up to 3 combinations pay out).
    """
    ...
    def run_backtest(
        ...
        wide_lookup: Dict[int, List[Tuple[int, int, float]]],
    ) -> WideBacktestResults:
        """
        ...
        Args:
            ...
            wide_lookup: Dict mapping race_id to list of (h1, h2, odds) tuples

        Returns:
            WideBacktestResults with all bet outcomes
        """
        results = WideBacktestResults()

        race_groups = test_df.groupby(RACE_ID_COL)
        logger.info(f"Backtesting wide on {len(race_groups)} races...")

        for race_id, race_df in race_groups:
            if race_id not in wide_lookup:
                continue

            wide_winners = wide_lookup[race_id]
            ...
            bets = self._evaluate_race(
                race_df, model, wide_winners, race_meta
            )
    ...
    def _evaluate_race(
        ...
        model: PositionProbabilityModel,
        wide_winners: List[Tuple[int, int, float]],
        race_meta: Dict,
    ) -> List[WideBetResult]:
        """Evaluate wide betting opportunities for a single race."""
        bets = []
        ...
        wide_probs = self.wide_calc.calculate_wide_probs(position_probs)

        top_wides = self.wide_calc.get_top_wides(
            wide_probs, n=self.max_bets_per_race
        )

        # Create set of winning combinations
        winning_combos = {
            frozenset({h1, h2}): odds
            for h1, h2, odds in wide_winners
        }

        # Derive actual top 3 from wide winners (3 pairs = C(3,2))
        all_horses = set()
        for h1, h2, _ in wide_winners:
            all_horses.add(h1)
            all_horses.add(h2)
        top_3 = sorted(all_horses)[:3] if len(all_horses) >= 3 else list(all_horses)

        for horse_set, pred_prob in top_wides:
            ...
            won = pred_set_int in winning_combos
            actual_odds = winning_combos.get(pred_set_int, 0)

            payout = actual_odds if won else 0
            profit = payout - self.bet_unit

            decimal_odds = actual_odds / 100 if actual_odds > 0 else 0
            post_hoc_ev = pred_prob * decimal_odds if won else 0

            h1, h2 = sorted(horse_set)
            ...
            bet = WideBetResult(
                ...
                horse1=str(h1),
                horse2=str(h2),
                actual_1st=top_3[0] if len(top_3) > 0 else 0,
                actual_2nd=top_3[1] if len(top_3) > 1 else 0,
                actual_3rd=top_3[2] if len(top_3) > 2 else 0,
    ...
    def run_walkforward_backtest(
        ...
        train_months: int = 18,
        test_months: int = 3,
    ) -> Tuple[WideBacktestResults, List[Dict]]:
        """Run walkforward backtesting for wide."""
        logger.info("=" * 60)
        logger.info("WIDE WALKFORWARD BACKTEST")
        logger.info("=" * 60)

        # Load odds
        self.odds_loader.load_odds()
        wide_df = self.odds_loader.get_wide_results()

        # Create lookup: race_id -> list of (h1, h2, odds)
        wide_lookup = defaultdict(list)
        for _, row in wide_df.iterrows():
            race_id = row["レースID"]
            wide_lookup[race_id].append((
                row["wide_horse1"], row["wide_horse2"], row["wide_odds"]
            ))
        wide_lookup = dict(wide_lookup)
        ...
        all_results = WideBacktestResults()
        period_results = []
        ...
        for period in range(n_periods):
            ...
            model_train_mask = (df["_date"] >= train_start) & (df["_date"] < train_end)
            ...
            period_result = self.run_backtest(test_df, model, wide_lookup)
...
def main():
    ...
    # Random wide probability for 14 horses = 6 / C(14,2) ≈ 6.6%
    # (3 winning combos out of 91 total)
    random_prob = 3 / (14 * 13 / 2)
    our_hit_rate = results.hit_rate / 100
    print(f"\nRandom wide probability: {random_prob:.2%}")
